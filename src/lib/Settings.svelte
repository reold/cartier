<script lang="ts">
  ...
  import { AppState, useApp, OPFS } from "../store";
  ...
  const loadSettings = () => {
    notify("loading settings", true, 5, async (destroy) => {
...

      {
        ...
        settings["ropfs_used"] = `${opfsUsed.toFixed(1)} MB`;
      }
      ...
      if (!$AppState.offline) {
        ...
        settings["rfrontend_updated"] = updateInfo;
      }
...
>
  ...
  <div class="grid grid-cols-10 gap-2 btns-tab font-bold min-w-[95vw]">
    ...
    <button
      class="bg-orange-500 text-black svg-icon col-span-5"
      on:click={() => {
        window["cartier-server-url"] = "https://cartier-reold.koyeb.app";
        notify("waking up Koyeb server", true, 10, async (destroy) => {
          await requests.getRoot();
          destroy();
          loadSettings();
        });
    ...
    <button
      class="bg-primary text-black svg-icon col-span-5"
      on:click={loadSettings}
    >
      <svg
        xmlns="http://www.w3.org/2000/svg"
